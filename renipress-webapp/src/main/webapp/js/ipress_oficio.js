...
$(function () {
	...
	if (idIpress != null && idIpress != undefined){
		$('#idSolicitud').val(idIpress);
	}
    ...
    $.ajax({
        url: "ipress.htm?action=selectPais",
        dataType: "json",
		async:false,
    ...
    $.ajax({
        url: "ipress.htm?action=selectModulo",
        dataType: "json",
		async:false,
        success: function (result) {
            listaModulos = result;
    ...
    //CargarDepartamentos('ipress.htm?action=selectUbigeo&tipo=DE', $('#cboDepartamento'), etiquetasControles.departamento); //comentado por dlarico
    CargarDepartamentos('ipress.htm?action=selectUbigeo&tipo=DE', $('#cboDepRA'), etiquetasControles.departamento);
    CargarDepartamentos('ipress.htm?action=selectUbigeo&tipo=DE', $('#cboDepDE'), etiquetasControles.departamento);
    CargarDepartamentos('ipress.htm?action=selectUbigeo&tipo=DE', $('#cboDepartamentoDA'), etiquetasControles.departamento);
    ...
    setTimeout(function(){
        if (typeof idSolConsultar === 'undefined'
			...
			ActualizarSolicitud(idIpress, idOpcion);			
        }
    }, 2000);
...
function ActualizarSolicitud(idSolConsultar, idOpcion) {
    beginCargando(); 
    $.getJSON("ipress.htm?action=cargarConsultaSolicitudJSON&idSol=" + idSolConsultar,function(data){
		...
		*/
		...
		$('#idSolicitud').val(idSolConsultar);
        ...
        */
		...
		$('#idSolicitudIPRESS').val(idSolConsultar);
        ...
        //DATOS DE PERSAONA JURIDICA
            ...
            if (objSolicitudDS.DP_CODUBIGEO !== undefined){
				console.log(objSolicitudDS.DP_CODUBIGEO);
			...
			*/
            ...
            if (objSolicitudDS.TI_PERSONAPROP_SI === 3) {
                $('#lblNumeroSerie').html("N&Uacute;MERO DE SERIE DEL RUC "+objSolicitudDS.NU_RUCPROP_SI+" (*)");
            }else{
                $('#lblNumeroSerie').html("N&Uacute;MERO DE SERIE DEL RUC "+objSolicitudDS.NU_RUCPROP_SI);
            }
			...
			// CMIC 24/11/2015
			//--seleccionar combos (cboDepRA,cboProRA,cboDisRA) 
			var ubigeoDatosEstablecimiento = [];
            if (objSolicitudDS.DE_IDUBIGEO_DE !== undefined){
				console.log(objSolicitudDS.DE_IDUBIGEO_DE);
                ubigeoDatosEstablecimiento = objSolicitudDS.DE_IDUBIGEO_DE.split(',');
                //setTimeout(function () {
                        $('#cboDepDE').val(ubigeoDatosEstablecimiento[1].substr(0, 2));
                        $('#cboDepDE').trigger('change');
                        //setTimeout(function () {
                                $('#cboProvDE').val(ubigeoDatosEstablecimiento[1].substr(0, 4));
                                $('#cboProvDE').trigger('change');
                                //setTimeout(function () {
                                        $('#cboDistDE').val(ubigeoDatosEstablecimiento[0]);
                                        $('#cboDistDE').trigger('change');
                                //}, 3000);
                        //}, 2000);
                //}, 1000);
            }
            ...
            //var cargarSelectAutSanitaria = function () {
				...
				try {
					$.ajax({
						dataType: "json",
						async:false,
						'url': 'ipress.htm?action=selectAutSanUbi&ubigeo=' + $('#cboDistDE').val(),			
            ...
            if(objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "10" || objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "3" || objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "4" ||
                ...
                } else if (objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "1" || objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "7" || objSolicitudDS.CO_IDINSTITUCIONESTABL_SI === "14") {//caso IGSS (MINSA) ó caso GOBIERNO REGIONAL                    
            ...
            */
			...
			$('#txtTelEmerDatosDE').val( $('#txtTelEmerDatosDE').val().trim() );
		...
		*/
		...
		$('#txtNroEstDA').attr('disabled',true);
		$('#cboCuentaPoblacionDA').attr('disabled',true);	
		$('#txtEspecificarDA').attr('disabled',true);
		$('#cmb_estadoActual').attr('disabled',true);
		...
		$('#dat_fd_ipress').val(objSolicitudDS.DE_FECHA_INI);
		$('#dat_fh_ipress').val(objSolicitudDS.DE_FECHA_FIN);
		$('#txt_motivoRechazo').val(objSolicitudDS.DE_MOTIVO);
		$('#dat_fc_ipress').val(objSolicitudDS.DE_FECHA_CIE);
...
function CargarSexo(select) {
    try {   
		...
		$.ajax({
			dataType: "json",
			async:false,
			url: 'ipress.htm?action=selectSexo',			
			success:  function (json) {
				sexoCache = json;
...
function ValidarDNI() {
    ...
    try {
        if ($('#txtDocumento').val() !== '' && $('#cboSexo').val() !== '' && $('#cboFecNac').val() !== '') {
            $.ajax({
                url: "ipress.htm?action=selectDNI&dni=" + $('#txtDocumento').val() + "&sexo=" + $('#cboSexo').val() + "&fecNac=" + $('#cboFecNac').val() + "&cboDocumento=" + $('#cboDocumento').val(),
                dataType: "json",
                success: function (result) {					
					if (result.preNombres === undefined && JSON.parse(result.coError) === 1050){
						$('#lblTituloPopGenerico').text('Error');
						$('#lblConenidoPopGenerico').text('La persona no puede ser menor de edad');
						$('#popGenericoRegistro').modal("show");
					}
...
function ValidarDNI1() {
    ...
    try {
        if ($('#txtNroDocRA').val() !== '' && $('#cboSexoRA').val() !== '' && $('#txtFecNacRA').val() !== '') {
            $.ajax({
                url: "ipress.htm?action=selectDNI&dni=" + $('#txtNroDocRA').val() + "&sexo=" + $('#cboSexoRA').val() + "&fecNac=" + $('#txtFecNacRA').val() + "&cboDocumento=" + $('#cbdoTipoDocRA').val(),
                dataType: "json",
                success: function (result) {
					if (result.preNombres === undefined && JSON.parse(result.coError) === 1050){
						$('#lblTituloPopGenerico').text('Error');
...
function ValidarDNI2() {
    ...
    try {
        if ($('#txtNroDocRRLL').val() !== '' && $('#cboSexoRRLL').val() !== '' && $('#txtFecNacRRLL').val() !== '') {
            $.ajax({
                url: "ipress.htm?action=selectDNI&dni=" + $('#txtNroDocRRLL').val() + "&sexo=" + $('#cboSexoRRLL').val() + "&fecNac=" + $('#txtFecNacRRLL').val() + "&cboDocumento=" + $('#cboTipoDocRRLL').val(),
                dataType: "json",
                success: function (result) {
					if (result.preNombres === undefined && JSON.parse(result.coError) === 1050){
						$('#lblTituloPopGenerico').text('Error');
...
function ValidarDNI3() {
    ...
    try {
        if ($('#txtNroDocRAS').val() !== '' && $('#cboSexoRAS').val() !== '' && $('#txtFecNacRAS').val() !== '') {
            $.ajax({
                url: "ipress.htm?action=selectDNI&dni=" + $('#txtNroDocRAS').val() + "&sexo=" + $('#cboSexoRAS').val() + "&fecNac=" + $('#txtFecNacRAS').val() + "&cboDocumento=" + $('#cboDocIdenRAS').val(),
                dataType: "json",
                success: function (result) {
					if (result.preNombres === undefined && JSON.parse(result.coError) === 1050){
						$('#lblTituloPopGenerico').text('Error');
...
function ValidarDNI4() {
    ...
    try {
        if ($('#txtNroDocExcel').val() !== '' && $('#CboSexoExcel').val() !== '' && $('#txtFecNacExcel').val() !== '') {
            $.ajax({
                url: "ipress.htm?action=selectDNI&dni=" + $('#txtNroDocExcel').val() + "&sexo=" + $('#CboSexoExcel').val() + "&fecNac=" + $('#txtFecNacExcel').val() + "&cboDocumento=" + $('#cboTipoDocExcel').val(),
                dataType: "json",
                success: function (result) {					
					if (result.preNombres === undefined && JSON.parse(result.coError) === 1050){
						$('#lblTituloPopGenerico').text('Error');
...
function ConfigControles() {
	...
	/* comentado por dlarico	
        ...
        */
		...
		// CMIC 24/11/2015
		$('#cboDepDE').change(function () {
            CargarComboDefecto($('#cboProvDE'), etiquetasControles.provincia);
            CargarComboDefecto($('#cboDistDE'), etiquetasControles.distrito);
            CargarProvincias(this.value, $('#cboProvDE'), etiquetasControles.provincia);            
        });

        $('#cboProvDE').change(function () {
            CargarComboDefecto($('#cboDistDE'), etiquetasControles.distrito);
            CargarDistrito(this.value, $('#cboDistDE'), etiquetasControles.distrito);
        });
		
		$('#cboDistDE').change(function () {
            $('#cboIPAutoridadSanit').empty();
            $('#cboDistDE').val($('#cboDistDE').val());
			//CargarAutoridadSanitaria(this.value, $('#cboAutSanitaria'), 'Seleccione Autoridad Sanitaria');
			MostrarAutUbiego(this.value);
...
function CargarProvincias(depa, obj, rowDefault) {
    try {
        ...
        $.ajax({
            dataType: "json",
            async:false,
            url: 'ipress.htm?action=selectUbigeo&tipo=PR&codigoUbigeo='+ depa,		
            success:  function (json) {
                var select = obj;
                json.forEach(function (item, num) {
                    select.append($('<option/>', {
...
function CargarDistrito(prov, obj, rowDefault) {
    try {	
        $.ajax({
            dataType: "json",
            async:false,
            'url': 'ipress.htm?action=selectUbigeo&tipo=DI&codigoUbigeo='+ prov,			
            success:  function (json) {
                var select = obj;
                json.forEach(function (item, num) {
                    select.append($('<option/>', {
...
function CargarAutoridadSanitaria(ubigeo, obj, rowDefault, functionEval) {
    try {
        $.ajax({
            dataType: "json",
            async:false,
            'url': 'ipress.htm?action=selectAutSanUbi&ubigeo=' + ubigeo,			
            success:  function (json) {
                flagAutSanit = true;
                var select = obj;
                json.forEach(function (item, num) {
                ...
                if ($(this).data('value') === 1) {
                    ...
                    $.ajax({
                        ...
                        success: function (
                            ...
                            $.ajax({
                                url: "ipress.htm?action=actualizarAdjuntosSolicitud&idSolicitud=" + result.P_IDSOLICITUD,
                                dataType: "json",
                                data: listaObj,
                                success: function (result) {
...
/*
	...
	for(var i = 0; i < listaActividadEstado.length; i++){
		if (arrayActividadEstado.length > 0){
			arrayActividadEstado += ',';
		}
		arrayActividadEstado += listaActividadEstado[i].idcodigo;
	}
	
	for(var i = 0; i < listaUpssEstado.length; i++){
		if (arrayUpssEstado.length > 0){
			arrayUpssEstado += ',';
		}
		arrayUpssEstado += listaUpssEstado[i].idcodigo;
	}
    ...
    $.ajax({
        url: "ipress.htm?action=insertDatosGeneralesSol",
        dataType: "json",
        type: "POST",
        data: $('#frmDatosPropietario').serialize() + '&'
				...
				// CMIC 25/11/2015
				+ '&flag_estado=' + ( $('#cmb_estadoActual').val() == undefined ? '0' : '1' )
				+ '&cmb_estadoActual=' + $('#cmb_estadoActual').val()
				+ '&cmb_nuevoEstado=' + $('#cmb_nuevoEstado').val()
				+ '&dat_fd_ipress=' + $('#dat_fd_ipress').val()
				+ '&dat_fh_ipress=' + $('#dat_fh_ipress').val()
				+ '&dat_fc_ipress=' + $('#dat_fc_ipress').val()
				+ '&txt_motivoRechazo=' + $('#txt_motivoRechazo').val()
				+ '&arrayUpssEstado=' + arrayUpssEstado
				+ '&arrayActividadEstado=' + arrayActividadEstado
        ,
        success: function (result) {
			...
			$.ajax({
				url: "ipress.htm?action=actualizarAdjuntosSolicitud&idSolicitud=" + $('#idIPRESS').val(),
				dataType: "json",
				data: listaObj,
				success: function (result) {
					console.log(result);
            ...
            $.ajax({
                url: "ipress.htm?action=selectUPDCalculoRRHH&idSolicitud="+$('#idIPRESS').val(),
                dataType: "json",
                success: function (result) {
                    console.log(result);
                    endCargando();
					
					$('#lblConenidoPopGenerico').text('Se ha registrado la información ingresada');
                    ...
                    $('#popGenericoRegistro').modal({
                        ...
                        if ($(this).data('value') === '1') {
							$(location).prop( 'href', 'ipress.htm?action=mostrarBuscar' ) ;
                        } else {
                            $(location).prop( 'href', 'ipress.htm?action=mostrarBuscar' ) ;
                        }
                    });
...
function MostrarTipDoc() {
    $.ajax({
        url: "ipress.htm?action=selectTipDoc",
        dataType: "json",
        data: $('#frmDatosPropietario').serialize(),
        success: function (result) {
...
function MostrarAutoridadSanit() {
    $.ajax({
        url: "ipress.htm?action=selectAutSan",
        dataType: "json",
        success: function (result) {
...
function MostrarProfesion() {
    $.ajax({
        url: "ipress.htm?action=selectProfesion",
        dataType: "json",
        success: function (result) {
...
function MostrarTipoEstablecimiento() {
    $.ajax({
        url: "ipress.htm?action=selectTipoClas",
        dataType: "json",
        success: function (result) {
...
function MostrarTipoContrato() {
    $.ajax({
        url: "ipress.htm?action=selectTipoContrato",
        dataType: "json",
        success: function (result) {
...
function MostrarEstablecimiento(tipo, idSolicitud) {
    ...
    if (tipo !== '') {
        $.ajax({
            url: "ipress.htm?action=selectClasEst&tipoEst=" + tipo + "&idSolici=" + idSolicitud,
            dataType: "json",
            success: function (result) {
...
function MostrarAutUbiego(ubigeo) {
    ...
    $.ajax({
        url: "ipress.htm?action=selectAutSanUbigeo&ubigeo=" + ubigeo,
        dataType: "json",
        success: function (result) {
...
function MostrarInstitucion() {
    ...
    if (tipoPersona !== 0) {
        $.ajax({
            //url: "ipress.htm?action=selectIntitucion",
            url: "ipress.htm?action=selectIntitucionPorPorsona&tipoPersona=" + tipoPersona, //Agregado por dlarico
            dataType: "json",
			async:false,
            success: function (result) {
...
function MostrarRed(idInstitucion) {
    ...
    if(idInstitucion === "10" || idInstitucion === "3" || idInstitucion === "4" ||
       ...
       if (idInstitucion === "2") {//caso ESSALUD
            $('#divRed').css('display', 'block');
            $('#divMicroRed, #divUE, #divEstClasEst').css('display', 'none');
        } else if (idInstitucion === "1" || idInstitucion === "7" || idInstitucion === "14") {//caso IGSS (MINSA) ó caso GOBIERNO REGIONAL
            $('#divRed, #divMicroRed, #divUE, #divEstClasEst').css('display', 'block');
    ...
    //        MostrarUniEjecutora();
        ...
        //--------------
        $.ajax({
            url: "ipress.htm?action=selectRed" + "&as=" +idAutoridad + "&institucion=" + idInstitucion,
            dataType: "json",
			async:false,
            success: function (result) {
...
function MostrarMicroRed(red) {
    ...
    if ($.trim(red) !== null && $.trim(red) !== "" && red !== undefined) {
        $.ajax({
            url: "ipress.htm?action=selectMicroRed&cboRedEst=" + red + "&as=" + idAutoridad,
            dataType: "json", 
			async : false,
            success: function (result) {
...
function MostrarUniEjecutora() {
    ...
    $('#cboUniEjeEst').empty();
    $.ajax({
        url: "ipress.htm?action=selectUnEjecutora&cboInstitucionEst="+idInstitucion+"&as="+$('#cboAutSanitaria').val(),
        dataType: "json",
		async:false,
        success: function (result) {
...
function MostrarEstabClas() {
    ...
    $.ajax({
        url: "ipress.htm?action=selectEstClas&as="+idAutoridad,
        dataType: "json",
		async:false,
        success: function (result) {
...
function MostrarGrupoEtareo(ubigeo) {
    $.ajax({
        url: "ipress.htm?action=selectGrupoEtareo",
        dataType: "json",
        success: function (result) {
...
function MostrarTipoAtencion(ubigeo) {
    $.ajax({
        url: "ipress.htm?action=selectTipoAtencion",
        dataType: "json",
        success: function (result) {
...
// CMIC 24/11/2015
var arrUPSSEstadoDAdicional = [];
var arrUPPSSEstadoDASeleccion = [];
function SelActividadDG(item, valor) {
    if ($(item).is(":checked")) {
        arrUPSSEstadoDAdicional.push(valor);
    } else {
        arrUPSSEstadoDAdicional = EliminarASimple(arrUPSSEstadoDAdicional, valor);
    }
}

var arrActividadEstadoDAdicional = [];
function SelActividadDG(item, valor) {
    if ($(item).is(":checked")) {
        arrActividadEstadoDAdicional.push(valor);
    } else {
        arrActividadEstadoDAdicional = EliminarASimple(arrActividadEstadoDAdicional, valor);
    }
}
...
function CargarParametros() {
    try {
        $.getJSON('ipress.htm?action=selectListaParametros', function (json) {
            lP = json;
        });
    } catch (e) {
        console.log(e);
    ...
    if (files.length > 0) {
        if (files[0].size > 10000000) {
            ...
            var ajaxRequest = $.ajax({
                type: "POST",
                url: "ipress.htm?action=adjuntar",
                contentType: false,
                processData: false,
                data: data
            });
...
$(function () {
    dsGdSolicitudes = new kendo.data.DataSource({
        transport: {
            read: function (options) {
                var idSolicEditar = $("#idIPRESS").val();
                var ruta = "";
                if (idSolicEditar === "" || idSolicEditar === undefined) {
                    ruta = "ipress.htm?action=cargarAdjuntosSolicitud&idAS=" + $('#cboIPAutoridadSanit').val();
                } else {
                    ruta = "ipress.htm?action=cargarAdjuntosSolicitud&idSol=" + idSolicEditar+"&idAS=" + $('#cboIPAutoridadSanit').val();
                }
                $.ajax({
                    url: ruta,
                    dataType: "json",
    ...
    $('#btnCargarXlsRRHH').click(function () {
        ...
        beginCargando();
        $.ajax({
            url: "ipress.htm?action=evaluarExcel",
            type: "POST",
            contentType: false,
            processData: false,
            data: data,
    ...
    dsGdActividadesDA = new kendo.data.DataSource({
        transport: {
            read: function (options) {
                $.ajax({
                    url: "ipress.htm?action=selectACTDA&idSol=" + $('#idSolicitud').val(),
                    dataType: "json",
					async : false,
                    success: function (result) {
	...
	dsGdEspecialidadSolicitudDA = new kendo.data.DataSource({
        transport: {
            read: function (options) {
                $.ajax({
                    url: "ipress.htm?action=selectEspecialidadSolicitud&idSol=" + $('#idSolicitud').val(),
                    dataType: "json",
                    success: function (result) {
    ...
    dsGdConsultaExt = new kendo.data.DataSource({
        transport: {
            read: function (options) {
                $.ajax({
                    url: "ipress.htm?action=selectACTDA",
                    dataType: "json",
                    success: function (result) {
                        options.success(result);
                    },
...
function CargarUPSDAdicional() {
    UPSDatosAdicionales = [];
    $.ajax({
            url : "ipress.htm?action=selectUPSDA"+"&P_TIPOESTABLE="+$('#cboTipoEstablecimiento').val()+"&idSolici="+$('#idSolicitud').val(),
            dataType : "json",
            success : function(result) {
                var newArregloUPSS = [];
                UPSTemp = [];
...
function CargarUPSSDAdicional() {
    ...
    $.ajax({
        url : "ipress.htm?action=selectUPSSDA"+"&P_TIPOESTABLE="+$('#cboTipoEstablecimiento').val()+"&idSolici="+$('#idSolicitud').val(),
        dataType : "json",
        success : function(result) {
            var newArregloUPSS = [];
            UPSSTemp = [];
...
/**
	CMIC 23/11/2015
*/
function retornaUPSSDAdicionalEstado() {
    return UPSSDatosAdicionalesEstado;
}
//------
    ...
    //if(rrhhListaValidos.length!==0){
        ...
        if(valido){
            ...
            if(rrhhListaValidos.length===0){
                ...
                setDataSession(jsonString, 'ipress.htm');//default-js.jsp 
                //location.href='ipress.htm?action=GeneraExcel&arregloRRHH='+jsonString;
...
function SeleccionarItemsEspecialidad(arr, arrE, arrRNE) {
    ...
    if (arr.length > 0) {
        $.ajax({
            url: "ipress.htm?action=selectEspecialidadColegio&colegios=" + ( arr.length > 0 ? arr.toString() : '' ),
            dataType: "json",
            success: function (result) {
                objtEspec = result;
...
function ValidaImprimirPDF() {
    ...
    if (flagValid) {
		...
		beginCargando();
        $.getJSON('ipress.htm?action=selectUPSSEstado&solIPRESS=' + $('#idSolicitudIPRESS').val(), function (json) {
			endCargando();
            ...
            if (ValidarFormularios()) {
                ...
                $('#confirmEnviarSolicitud').modal({
                    ...
                    if ($(this).data('value') === 1) {
                        beginCargando();
                        $.ajax({
                            url: "ipress.htm?action=ActualizarTarea&idSolicitudIPRESS=" + $('#idSolicitudIPRESS').val(),
                            dataType: "json",
                            success: function (result) {
                                endCargando();
                                setTimeout(function () {
...
function imprimirPDF() {
    ...
    var direccionEst = $('#txtUrSOL').val() + " " + 
            ...
            document.getElementById('cboDistritoDA').options[document.getElementById('cboDistritoDA').selectedIndex].text + " "; 			
             ...
             */    
    $('#ifrReporte').attr('src',
            'ipress.htm?action=imprimirPDF&txtRucRA=' + $('#txtRucRA').val()
...
function CargarEspecialidadColegio(arr) {
    if (arrColegio.length >= 0) {
        ...
        $.ajax({
            url: "ipress.htm?action=selectEspecialidadColegio&colegios=" + ( arrColegio.length >= 0 ? arr.toString() : '' ),
            dataType: "json",
			async : false,
            success: function (result) {
...
function CargaObjUPSS() {
    $.ajax({
        url: "ipress.htm?action=selectUPSSEsta" + "&P_SOL=" + $('#idSolicitud').val(),
        dataType: "json",
        success: function (result) {
            objtUPSS = result;
            setTimeout(function () {
...
function deleteEstablecimiento(tipo) {
    beginCargando();
    $.ajax({
        url: "ipress.htm?action=deleteEstablecimiento" + "&idsolicitud=" + $('#idSolicitud').val() + "&idtipo=" + tipo,
        dataType: "json",
		async : false,
        success: function (result) {
            endCargando();
...
function deleteUpss(idupss) {
    beginCargando();
    $.ajax({
        url: "ipress.htm?action=deleteUpss" + "&idsolicitud=" + $('#idSolicitud').val() + "&idupss=" + idupss,
        dataType: "json",
		async : false,
        success: function (result) {
            endCargando();
...
function CargaObjColProf() {
    $.ajax({
        url: "ipress.htm?action=selectColegioProf",
        dataType: "json",
        success: function (result) {
            objtColProf = result;
...
function CargaClasifEstab() {
    $.ajax({
        url: "ipress.htm?action=selectClasEst&tipo=1",
        dataType: "json",
        success: function (result) {
            objtClasEst = result;
        },
...
function CargaCompetencia() {
    $.ajax({
        url: "ipress.htm?action=selectCompetencias&tipo=CO",
        dataType: "json",
        success: function (result) {
...
function CargaCapacitacion() {
    $.ajax({
        url: "ipress.htm?action=selectCompetencias&tipo=CA",
        dataType: "json",
        success: function (result) {
...
function CargaEntrenamiento() {
    $.ajax({
        url: "ipress.htm?action=selectCompetencias&tipo=EN",
        dataType: "json",
        success: function (result) {
...
function mostrarSubsanarSolicitud(){
	var form = $('#form_solicitud');		
	form.attr('action', 'ipress.htm?action=cargarSolicitud'+'&idEntidad='+idSolConsultar +'&idSol='+idSolConsultar );
	form.attr('method', 'post');	        	
	form.submit();
} 
...
function valDireccionEst() {
	var sw = 0;
    $.ajax({
        url : "ipress.htm?action=valDireccionEst&"+$('#frmDatosEstablecimiento').serialize(),
        dataType : "json",		
        type: 'POST',
		async : false,
        success : function(result) {
...
function fu_getRuc(tipo) {
    ...
    if (ruc !== "") {
        ...
        if (cantidadCaracteres === 11) {
            beginCargando();
            $.ajax({
                url: 'ipress.htm?action=buscarRuc',
                cache: false,
                async: true,
                type: 'POST',
                data: {'ruc': ruc},
...
function fu_getIdUbigeo(ubigeo) {
    var id;
    $.getJSON('ipress.htm?action=selectUbigeo&tipo=ID&codigoUbigeo=' + ubigeo, function (json) {
        json.forEach(function (item, num) {
            id = item.ID;
            if (id !== undefined) {
                $('#cboDisRA').val(id);
...
function fu_goValidadRucSerieDE(){
    ...
    if(ruc !== ''){
        beginCargando();
        $.ajax({
                url: 'ipress.htm?action=buscarRucSerie',
                cache: false,
                async: true,
                type: 'POST',
