/* global idSolConsultar, idOpcion, attr, upssActivo, objetoReq, idUPPSIcon, moduloActivo, dsGdClasificaEstab, dsGdColegioProfesional, dsGdEspecialidad, dsGdCapacitacion, dsGdEntrenamiento, dsGdUPSS, kendo, dsGdRRHH, dsGdRRHHNoValido, dsGdActividadesDA, codUniIpress, coAccesoEnvio, codProceso */
...
function LimpiarFiltrosDA(){
    $('#txtBuscarUPSDA, #txtBuscarUPSSDA, #txtBuscarActDA').val('');
    console.log('limpiando filtros');
}

$(function () {
    console.log('Iniciar Carga actualizacionIpress');
    beginCargando();
    CargarParametros();
    LimpiarFiltrosDA();
    CargaControles();
    ...
    setTimeout(function(){        
        if (typeof idSolConsultar === 'undefined' || idSolConsultar === 0 || idSolConsultar === ''){
            fu_actualizarDatos(codUniIpress, idOpcion, coAccesoEnvio, codProceso);
            //endCargando();
        }else{
...
function fu_actualizarDatos(codUniIpress, idOpcion, coAccesoEnvio, codProceso){
    beginCargando(); 
    $.getJSON("actualizacionRetiro.htm?action=cargarIpressJSON&codUniIpress="+codUniIpress,function(data){
        idopcionValidar = idOpcion;
        console.log("idopcionValidar " +idopcionValidar);
        objSolicitud = data;
        //-----INICIO DATOS PRIMERA PESTAÑA
        objSolicitudDG = objSolicitud['P_CURSORIPRES'][0];
        objSolicitudDS = objSolicitud['P_CURSOR'][0];
        objSolicitudRH = objSolicitud['P_CURSORSOLRRHH'];
        
        //-----INICIO DATOS PRIMERA PESTAÑA
        console.log("INICIO DATOS PRIMERA PESTAÑA");
        $('#txtUniCoIpress').val(objSolicitudDG.CO_UNICOIPRESS_I);
        $('#txtRucProp').val(objSolicitudDG.NU_RUCPROP_I);
        $('#txtRazonSocialProp').val(objSolicitudDG.DE_RAZONSOCIALPROP_I);
        $('#txtComercialProp').val(objSolicitudDG.NO_COMERCIALPROP_I);
        $('#txtDireccionProp').val(objSolicitudDG.DE_DIRECCIONPROP_I);
        $('#txtNomAutoridadSanitaria').val(objSolicitudDG.DE_NOMBREAUTORIDAD_I);
        $('#txtCodAutoridadSanitaria').val(objSolicitudDG.CO_IDAUTSANITARIA_I);
        
        $('#idSolicitud').val("");
        $('#coAccesoEnvio').val(coAccesoEnvio);
        $('#codProceso').val(codProceso);
        
        //---Carga Select combos (cboDepartamentoDA, cboProvinciaDA, cboDistritoDA) ----
        console.log("CARGAR COMBO UBIGEO");
        if(objSolicitudDS.DP_CODUBIGEO!==undefined){
            ubigeoDocAdjuntos = objSolicitudDS.DP_CODUBIGEO.split(',');
            $('#cboDepartamentoDA').val(ubigeoDocAdjuntos[1].substr(0, 2));
            $('#cboDepartamentoDA').trigger('change');
                $('#cboProvinciaDA').val(ubigeoDocAdjuntos[1].substr(0, 4));
                $('#cboProvinciaDA').trigger('change');
                    $('#cboDistritoDA').val(ubigeoDocAdjuntos[0]);
                    $('#cboDistritoDA').trigger('change');
        }
        console.log("FIN DATOS PRIMERA PESTAÑA");
        ...
        if(coAccesoEnvio===1){
            //-----------SEGUNDA PESTAÑA
            console.log("INICIO SEGUNDA PESTAÑA");
                ...
                if (objSolicitudDS.TI_PERSONAPROP_SI === 1) {
                    ...
                    $('#txtNroDocRAAux').val(objSolicitudDS.NU_DOC_PP);
                    $('#txtFecNacRA').val(objSolicitudDS.FE_FECHANAC_PP);
                    $('#txtFecNacRAAux').val(objSolicitudDS.FE_FECHANAC_PP);
                    $('#txtAPaternoRA').val(objSolicitudDS.AP_PATERNO_PP);
                    $('#txtAPaternoRAAux').val(objSolicitudDS.AP_PATERNO_PP);
                    $('#txtAMaternoRA').val(objSolicitudDS.AP_MATERNO_PP);
                    $('#txtAMaternoRAAux').val(objSolicitudDS.AP_MATERNO_PP);
                    $('#txtACasadaRA').val(objSolicitudDS.AP_CASADA_PP);
                    $('#txtACasadaRAAux').val(objSolicitudDS.AP_CASADA_PP);
                    $('#txtNombresRA').val(objSolicitudDS.DE_NOMBRE_PP);
                    $('#txtNombresRAAux').val(objSolicitudDS.DE_NOMBRE_PP);

                    $('#cbdoTipoDocRA').val(objSolicitudDS.TI_TIPODOC_PP);
                    $('#cbdoTipoDocRAAux').val(objSolicitudDS.TI_TIPODOC_PP);
                    $('#cboPaisRA').val(objSolicitudDS.CO_IDPAIS_PP);
                    $('#cboPaisRAAux').val(objSolicitudDS.CO_IDPAIS_PP);
                    $('#cboSexoRA').val(objSolicitudDS.TI_IDSEXO_PP);
                    $('#cboSexoRAAux').val(objSolicitudDS.TI_IDSEXO_PP);
                ...
                if (objSolicitudDS.TI_PERSONAPROP_SI === 2 || objSolicitudDS.TI_PERSONAPROP_SI === 3) {
                    ...
                    $('#txtNComercialPJRAAux').val($.trim(objSolicitudDS.NO_COMERCIALPROP_SI));
                }
                ...
                var cargarSelectEstableTipo = function () {
                    ...
                    $('#cboTipoEstablecimientoAux').val(objSolicitudDS.TI_TIPOESTABL_SI);
                ...
                $('#txtNuSerieDEAux').val(objSolicitudDS.NU_SERIEESTABL_SI);
                if (objSolicitudDS.TI_PERSONAPROP_SI === 3) {
                    $('#lblNumeroSerie').html("N&Uacute;MERO DE SERIE DEL ESTABLECIMEINTO "+objSolicitudDS.NU_RUCPROP_SI+" (*)");
                }else{
                    $('#lblNumeroSerie').html("N&Uacute;MERO DE SERIE DEL ESTABLECIMEINTO "+objSolicitudDS.NU_RUCPROP_SI);
                }
                $('#txtRazonSocialDE').val(objSolicitudDS.NO_RAZONSOCIALESTABL_SI);
                $('#txtNComercialDE').val(objSolicitudDS.NO_COMERCIALESTABL_SI);
                $('#txtNComercialDEAux').val(objSolicitudDS.NO_COMERCIALESTABL_SI);
                //---DIRECCIÓN----------------------------------------------------
                $('#txtViaDetDE').val(objSolicitudDS.DE_VIA_DE);
                $('#txtViaDetDEAux').val(objSolicitudDS.DE_VIA_DE);
                $('#txtNroDE').val(objSolicitudDS.NU_NUMERO_DE);
                $('#txtNroDEAux').val(objSolicitudDS.NU_NUMERO_DE);
                $('#txtNroPisoDE').val(objSolicitudDS.NU_PISO_DE);
                $('#txtNroPisoDEAux').val(objSolicitudDS.NU_PISO_DE);
                $('#txtNroDepDE').val(objSolicitudDS.NU_DEPARTAMENTO_DE);
                $('#txtNroDepDEAux').val(objSolicitudDS.NU_DEPARTAMENTO_DE);
                $('#txtNroIntDE').val(objSolicitudDS.NU_INTERIOR_DE);
                $('#txtNroIntDEAux').val(objSolicitudDS.NU_INTERIOR_DE);
                $('#txtMzDE').val(objSolicitudDS.DE_MANZANA_DE);
                $('#txtMzDEAux').val(objSolicitudDS.DE_MANZANA_DE);
                $('#txtLtDE').val(objSolicitudDS.DE_LOTE_DE);
                $('#txtLtDEAux').val(objSolicitudDS.DE_LOTE_DE);
                $('#txtKlDE').val(objSolicitudDS.DE_KILOMETRO_DE);
                $('#txtKlDEAux').val(objSolicitudDS.DE_KILOMETRO_DE);
                $('#txtUrDE').val(objSolicitudDS.DE_URBANIZ_DE);
                $('#txtUrDEAux').val(objSolicitudDS.DE_URBANIZ_DE);
                $('#txtRefDE').val(objSolicitudDS.DE_REFERENCIA_DE);
                $('#txtRefDEAux').val(objSolicitudDS.DE_REFERENCIA_DE);
                $('#cboViaDE').val(objSolicitudDS.CO_IDTIPOVIA_DE);
                $('#cboViaDEAux').val(objSolicitudDS.CO_IDTIPOVIA_DE);
                ...
                $('#cboAutSanitariaAux').val(objSolicitudDS.CO_IDAUTSANITARIAESTABL_SI);
                    ...
                    try {
                        var timeAutInstitucion = setInterval(function () {
                            if (flagAutSanit === true) {
                                ...
                                $('#cboInstitucionEstAux').val(objSolicitudDS.CO_IDINSTITUCIONESTABL_SI);
                    ...
                    try {
                        var timeAut = setInterval(function () {
                            if (flagAutSanit === true) {
                                ...
                                $('#cboRedEstAux').val(objSolicitudDS.CO_IDRED_SI);
                        ...
                        var timeMicroRed = setInterval(function () {
                            $('#cboRedEst').val(objSolicitudDS.CO_IDRED_SI);
                                var EstClasEst = function () {
                                    ...
                                    $('#cboMicroRedEstAux').val(objSolicitudDS.CO_IDMICRORED_SI);
                    ...
                    try {
                        var timeAUniEje = setInterval(function () {
                            if (flagUniEje === true) {
                                ...
                                $('#cboUniEjeEstAux').val(objSolicitudDS.CO_IDUNIDADEJE_SI);
                        ...
                        var timeAutEstClas = setInterval(function () {
                            if (flagAutSanit === true) {
                                var Estclas = function () {
                                    ...
                                    $('#cboEstClasEstAux').val(objSolicitudDS.CO_IDCLAS_SI);
                                };
                ...
                $('#txtTelDatosDEAux').val(objSolicitudDS.NU_TELEFONOESTABL_SI);
                $('#txtTelEmerDatosDE').val(objSolicitudDS.NU_TELEFONOEMERGESTABL_SI);
                $('#txtTelEmerDatosDEAux').val(objSolicitudDS.NU_TELEFONOEMERGESTABL_SI);
                $('#txtRadioDatosDE').val(objSolicitudDS.DE_RADIOESTABL_SI);
                $('#txtRadioDatosDEAux').val(objSolicitudDS.DE_RADIOESTABL_SI);
                $('#txtFaxDatosDE').val(objSolicitudDS.NU_FAXESTABL_SI);
                $('#txtFaxDatosDEAux').val(objSolicitudDS.NU_FAXESTABL_SI);
                $('#txtEmailDatosDE').val(objSolicitudDS.DE_CORREOELECTRESTABL_SI);
                $('#txtEmailDatosDEAux').val(objSolicitudDS.DE_CORREOELECTRESTABL_SI);
                $('#txtReEmailDatosDE').val(objSolicitudDS.DE_CORREOELECTRESTABL_SI);
                $('#txtWebDatosDE').val(objSolicitudDS.DE_PAGINAWEBESTABL_SI);
                $('#txtWebDatosDEAux').val(objSolicitudDS.DE_PAGINAWEBESTABL_SI);
                $('#txtFecIniDatosDE').val(objSolicitudDS.FE_FECHAINICACTESTABL_SI);
                $('#txtFecIniDatosDEAux').val(objSolicitudDS.FE_FECHAINICACTESTABL_SI);
                $('#txtFecCreacionDatosDE').val(objSolicitudDS.FE_FECHACREACIONESTABL_SI);
                $('#txtFecCreacionDatosDEAux').val(objSolicitudDS.FE_FECHACREACIONESTABL_SI);
                $('#txtNroResolDatosDE').val(objSolicitudDS.NU_RESOLUCIONESTABL_SI);
                $('#txtNroResolDatosDEAux').val(objSolicitudDS.NU_RESOLUCIONESTABL_SI);
                ...
                $('#txtNroDocRASAux').val(objSolicitudDS.NU_DOC_PDM);
                $('#txtFecNacRAS').val(objSolicitudDS.FE_FECHANAC_PDM);
                $('#txtFecNacRASAux').val(objSolicitudDS.FE_FECHANAC_PDM);
                $('#txtAPaternoRAS').val(objSolicitudDS.AP_PATERNO_PDM);
                $('#txtAPaternoRASAux').val(objSolicitudDS.AP_PATERNO_PDM);
                $('#txtAMaternoRAS').val(objSolicitudDS.AP_MATERNO_PDM);
                $('#txtAMaternoRASAux').val(objSolicitudDS.AP_MATERNO_PDM);
                $('#txtACasadaRAS').val(objSolicitudDS.AP_CASADA_PDM);
                $('#txtACasadaRASAux').val(objSolicitudDS.AP_CASADA_PDM);
                $('#txtNombresRAS').val(objSolicitudDS.DE_NOMBRE_PDM);
                $('#txtNombresRASAux').val(objSolicitudDS.DE_NOMBRE_PDM);
                $('#txtNroColegiaturaRAS').val(objSolicitudDS.NU_COLEGIATURA_SI);
                $('#txtNroColegiaturaRASAux').val(objSolicitudDS.NU_COLEGIATURA_SI);
                $('#txtRegNacioRNERAS').val(objSolicitudDS.NU_RNEDIRMED_SI);
                $('#txtRegNacioRNERASAux').val(objSolicitudDS.NU_RNEDIRMED_SI);
                $('#txtTelRAS').val(objSolicitudDS.NU_TELEFONODIRMED_SI);
                $('#txtTelRASAux').val(objSolicitudDS.NU_TELEFONODIRMED_SI);
                $('#txtEmailRAS').val(objSolicitudDS.DE_CORREOELECTRDIRMED_SI);
                $('#txtEmailRASAux').val(objSolicitudDS.DE_CORREOELECTRDIRMED_SI);
                $('#txtReEmailRAS').val(objSolicitudDS.DE_CORREOELECTRDIRMED_SI);

                    $('#cboDocIdenRAS').val(objSolicitudDS.TI_TIPODOC_PDM);
                    $('#cboDocIdenRASAux').val(objSolicitudDS.TI_TIPODOC_PDM);
                    $('#cboPaisRAS').val(objSolicitudDS.CO_IDPAIS_PDM);
                    $('#cboPaisRASAux').val(objSolicitudDS.CO_IDPAIS_PDM);
                    $('#cboSexoRAS').val(objSolicitudDS.TI_IDSEXO_PDM);
                    $('#cboSexoRASAux').val(objSolicitudDS.TI_IDSEXO_PDM);
                    $('#cboColeProfRAS').val(objSolicitudDS.CO_IDCOLEGPROFDIRMED_SI);
                    $('#cboColeProfRASAux').val(objSolicitudDS.CO_IDCOLEGPROFDIRMED_SI);
                ...
                try{
                    setTimeout(function () {
                        if( objSolicitudDS.ADOLECENTES !== undefined){
                            ...
                            $('#txtHorarioAtenADODAAux').val(objSolicitudDS.HORARIOADOLECENTE);
                        }else{
                            $('#divHoraAdo').hide();
                            $('#chkL').prop( "checked", true);
                            $('#chkM').prop( "checked", true);
                ...
                /*****************************/
                //Ultimas cargas Segun el tipo de edicion
                if (idOpcion === 1) {
                        activarBotones();
                }
...
function ActualizarSolicitud(idSolConsultar, idOpcion) {
    beginCargando(); 
    $.getJSON("actualizacionRetiro.htm?action=cargarIpressJSON&codUniIpress="+idSolConsultar,function(data){
        idopcionValidar = idOpcion;
        console.log("idopcionValidar " +idopcionValidar);
        objSolicitud = data;
        //-----INICIO DATOS PRIMERA PESTAÑA
        objSolicitudDG = objSolicitud['P_CURSORIPRES'];
        objSolicitudDS = objSolicitud['P_CURSOR'];
        ...
        $('#txtCodAutoridadSanitaria').val(objSolicitudDG.CO_IDAUTSANITARIA_I);
            ...
            $('#idSolicitud').val(objSolicitudDG.CO_UNICOIPRESS_I);
...
//                $('#cboVia').val(objSolicitudDG.SOLNOTIF_IDVIA);
            ...
            $('#idSolicitudIPRESS').val(objSolicitudDS.CO_UNICOIPRESS_I);
        ...
        console.log("INICIO SEGUNDA PESTAÑA");
            ...
            //--seleccionar combos (cboDepRA,cboProRA,cboDisRA) 
            if (objSolicitudDS.DP_CODUBIGEO !== undefined){
                ubigeoDatosPropietario = objSolicitudDS.DP_CODUBIGEO.split(',');
                //setTimeout(function () {
                        $('#cboDepRA').val(ubigeoDatosPropietario[1].substr(0, 2));
...
function CargarValidadoresIPRESS() {
    ...
    $('#frmActualizarIPRESS').validate({
        submitHandler: function (form) {
            ...
            if (sumRequerido > 0) {
                LanzarPopGenerico('Informativo', '(Documentos adjuntos) Registre el archivo, Número y fecha de Documentos obligatorios, según corresponda.');
                return;
            }
            ...
            $('#popConfirmaSolicitud').modal({
                ...
                if ($(this).data('value') === 0){
                    ...
                    beginCargando();
                    $.ajax({
                        url: "actualizacionRetiro.htm?"
                             + RetornaDataForm('frmActualizarIPRESS')
                             + '&cboPais='+ $('#cboPais').val()
                        ...
                        success:function(result) {
                            ...
                            $('#lblConenidoPopGenerico').text('Se ha registrado la información de solicitud.');
                            $('#popGenericoRegistro').modal({
                                backdrop: 'static',
                                keyboard: false
                            }).one('click', '[data-value]', function (e) {
                                if ($(this).data('value') === 1) {
                                    mostrarActualizacionRetiro();
                                } else {
                                    mostrarActualizacionRetiro();
                                }
                            });
...
function ValidarFormularios() {
    ...
    if (!$('#frmDatosGrlAdicional').valid() || (arrUPSDAdicional.length === 0 ||arrActividadDAdicional.length === 0 || (typeof itemsSeleccionados !== 'undefined' && itemsSeleccionados !== 0))) {
...
function GuardarIpressActuRetiro() {
    ...
    if(idopcionValidar === 4){
        ...
        if (!$('#frmDatosGrlAdicional').valid() && (arrUPSDAdicional.length !== 0 || arrUPPSSDASeleccion.length !== 0 || arrActividadDAdicional.length !== 0)) {
            console.log('formulario no valido');
            LanzarPopGenerico('Informativo', '(Datos Adicionales) Revise las validaciones resaltadas en rojo');
            return false;
...
function MostrarMicroRed(red) {
    ...
    //});
    console.log('Fin MostrarMicroRed');
}
...
function SeleccionarItemsEspecialidad(arr, arrE, arrRNE) {
    ...
    if (arr.length > 0) {
        $.ajax({
            url: "solicitud.htm?action=selectEspecialidadColegio&colegios=" + arr.toString(),
            dataType: "json",
            success: function (result) {
                objtEspec = result;
...
function ValidaImprimirPDF() {
    ...
    if (!$('#frmDatosGrlAdicional').valid() && (arrUPSDAdicional.length !== 0 || arrActividadDAdicional.length !== 0)) {
        console.log('formulario no valido');
        resultado = false;
...
function imprimirPDF() {
    ...
    var direccionEst = $('#txtUrSOL').val() + " " + 
             ...
             */    
    $('#ifrReporte').attr('src',
            'solicitud.htm?action=imprimirPDF&txtRucRA=' + $('#txtRucRA').val()
            + '&txtRucPJRA=' + $('#txtRucPJRA').val()
            + '&txtFecIniDatosDE=' + $('#txtFecIniDatosDE').val()
            + '&txtNComercialPJRA=' + $('#txtNComercialDE').val()
            + '&txtRSocialPJRA=' + $('#txtRazonSocialDE').val()
            + '&txtAPaterno=' + $('#txtAPaterno').val()
            + '&txtAMaterno=' + $('#txtAMaterno').val()
            + '&txtNombres=' + $('#txtNombres').val()
            + '&txtTlf1=' + $('#txtTlf1').val()
            + '&txtTlf2=' + $('#txtTlf2').val()
            + '&txtCorreo1=' + $('#txtCorreo1').val()
            + '&txtCorreo2=' + $('#txtCorreo2').val()
            + '&idSolIPRESS=' + $('#idSolicitudIPRESS').val()
            + '&paramAutSanit=' + document.getElementById(nameCbo).options[document.getElementById(nameCbo).selectedIndex].text
            + '&paramDireccion=' + direccionEst
            + '&idSolicitud=' + $('#idSolicitud').val()
            + '&arrDocReqs=' + JSON.stringify(listaDocsAutSanit));
...
function ModificarDatosRRHHConfirm() {
    ...
    if (!flagBotonRegistro) {
        ...
        personaRRHHxls[0].DIA_NAC = arrFechaNac[0];//$('#txtFecNacExcel').val();
        ...
        personaRRHHxls[0].DIA_NAC = arrFechaNac[0];
        personaRRHHxls[0].MES_NAC = arrFechaNac[1];
        personaRRHHxls[0].ANIO_NAC = arrFechaNac[2];
        personaRRHHxls[0].NUM_DOCU = $('#txtNroDocExcel').val();
        personaRRHHxls[0].SEXO = $('#CboSexoExcel').val();
                ...
                success: function (data) {
                    ...
                    if ($.trim(estadoRuc) === "ACTIVO") {
                        ...
                        if (tiDoc === "1" && tipo === "PN") {
                            ...
                            $('#lblNumeroSerie').html("N&Uacute;MERO DE SERIE DEL ESTABLECIMEINTO "+nuruc);
                            fu_getDatosDniPN();
                            mostrar = true;
...
//------------------------------------------------------------
function mostrarActualizacionRetiro(){
	var form = $('#form_actualizacionRetiro');		
	form.attr('action', 'actualizacionRetiro.htm?action=cargarSolicitudActualizacion');
	form.attr('method', 'post');	        	
	form.submit();
} 
//----------------------------------------------------------
function validarModificacion(origen,auxiliar,flag){
    //CONSULTAR COMPRARAION ENTRE CAMPO ORIGEN Y AUXILIAR
    var valorOrigen = $.trim($('#'+origen).val().toUpperCase());
    var valorAuxiliar = $.trim($('#'+auxiliar).val().toUpperCase());
    var valorFlag = flag;
    if(valorOrigen !== valorAuxiliar){
        $('#'+origen).attr('class','form-control input-sm input-mostrar');
        if(flag!==''){
            $('#'+flag).val(valorFlag);
        }
    }else{
        $('#'+origen).attr('class','form-control input-sm');
    }
}