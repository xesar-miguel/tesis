...
function RecuperarCuenta() {
	$.ajax({
		method : "POST",
		url : "login.htm?action=valFrmEnviaEmail",
		data : {
...
function ValidaCambioClave() {
	...
	$.validator.addMethod("ValidaVacios",
			function(value, element) {
				if (value === '') {
					MostrarMensajeErr('msgV',
							'Verifique los siguientes errores:', true);
					return false;
				} else {
					return true;
				}
	...
	$.validator.addMethod("ValidaC", function(value, element) {
		if ($('#passwordNuevo').val() === $("#passwordConfirmado").val()
				&& $('#passwordNuevo').val().length > 3) {
			$("#divNuevaC").removeClass("has-error");
			$("#divConfirmar").removeClass("has-error");
			$("[for=passwordNuevo]").hide();
			$("[for=passwordConfirmado]").hide();
			MostrarMensajeErr('msgV', 'Verifique los siguientes errores:',
					false);
			return true;
		} else {
			if (($('#passwordNuevo').val() !== '' && $('#passwordConfirmado')
					.val() === '')
					|| ($('#passwordNuevo').val() === '' && $(
							'#passwordConfirmado').val() !== '')) {
				MostrarMensajeErr('msgV', 'Verifique los siguientes errores:',
						true);
				return true;
			} else {
