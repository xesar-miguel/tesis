...
function LanzarPopRedirect(msg) {
	$('#lblMsgPop').text(msg);
...
function RecuperarCuenta() {
	$.ajax({
		method : "GET",
		url : "login.htm?action=valFrmEnviaEmail&txtEmail=" + $('#txtEmail').val(),
	}).done(function(msg) {
		console.debug('se envio correo ' + msg);
	});
}
...
function ValidaCuenta() {
	$.ajax({
		method : "POST",
		url : "login.htm?action=valFrmValidaEnviaEmail",
		data : {
			txtEmail : $('#txtEmail').val()
		}
	}).done(function(msg) {
		console.debug(msg + "   mensaje respuesta");
		if (msg.usuario !== "0") {
			LanzarPopRedirect('Se envió un correo a ' + $('#txtEmail').val());
			RecuperarCuenta();
		} else {
			LanzarPop('El correo electrónico ' + $('#txtEmail').val() + ' ingresado no se encuentra registrado.');
		}
	});
}
