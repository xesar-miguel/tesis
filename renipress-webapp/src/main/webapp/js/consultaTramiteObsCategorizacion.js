...
var objSol;
function CargarSolicitud(idSol) {
	try {
		  $.getJSON('solicitud.htm?action=selectSolId&idSol='+idSol, function(json) {
objSol =json;
$('#cboSexoRA').val(objSol[0].TI_IDSEXO_PP);
$('#cbdoTipoDocRA').val(objSol[0].TI_TIPODOC_PP);
$('#cboPaisRA').val(objSol[0].TI_TIPODOC_PP);
$('#cboViaRA').val(objSol[0].CO_IDTIPOVIA_DP);

$('#cboTipoDocRRLL').val(objSol[0].TI_TIPODOC_PRL);
$('#cboPaisRRLL').val(objSol[0].CO_IDPAIS_PRL);
$('#cboProfesionRRLL').val(objSol[0].CO_IDPROFREPLEGAL_SI);
$('#cboSexoRRLL').val(objSol[0].TI_IDSEXO_PRL);

$('#cboTipoEstablecimiento').val(objSol[0].TI_TIPOESTABL_SI);
$('#cboViaDE').val(objSol[0].CO_IDTIPOVIA_DE);
$('#cboInstitucionEst').val(objSol[0].CO_IDINSTITUCIONESTABL_SI);
$('#cboDocIdenRAS').val(objSol[0].TI_TIPODOC_PDM);
$('#cboPaisRAS').val(objSol[0].CO_IDPAIS_PDM);
$('#cboSexoRAS').val(objSol[0].TI_IDSEXO_PDM);
$('#cboColeProfRAS').val(objSol[0].CO_IDCOLEGPROFDIRMED_SI);
$('#cboTipoAtencionDA').val(objSol[0].TI_TIPOATENCION_SI);
$('#cboGrupoEtarioDA').val(objSol[0].TI_GRUPOETARIO_SI);



		});
	} catch (e) {
		console.log('CargarSolicitud ' + e);
	}	
}
