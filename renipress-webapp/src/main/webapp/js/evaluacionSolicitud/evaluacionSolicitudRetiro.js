...
/* global idOpcion, idSolConsultar, kendo, codProceso, attr */
...
function CargarSolicitudAcceso(idSolConsultar, idProceso) {
    ...
    $.getJSON("evaluacion-solicitud.htm?action=cargarSolcitudAccesoJSON&idSolicitud="+idSolConsultar+"&codProceso="+idProceso,function(data){
        ...
        $("#idEntidad").val(objSolicitudDG.SOL_IDSOLICITUD);
        $("#hdd_cmb_tipo_solicitud").val(idProceso);
        //-----INICIO DATOS PRIMERA PESTAÑA
...
function RedireccionarPagAnterior(){
    ...
    $('#idEntidad').find('input').attr('disabled',false);
    $('#hdd_cmb_tipo_solicitud').find('input').attr('disabled',false);
    var form = $('#frmRedirect');
    form.attr('action','evaluacion-solicitud.htm?action=mostrarEvaluacionRetiro');
    form.submit();
}
